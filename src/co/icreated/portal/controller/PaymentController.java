...
public class PaymentController implements PaymentsApi, Authenticated {
    ...
    public ResponseEntity<List<PaymentDto>> getPayments() {
        ...
        List<PaymentDto> payments = paymentService.findBPartnerPayments(getSessionUser().getPartnerId());
        return ResponseEntity.ok(payments);
    }

    /**
     * Get Credit Card Payment Here dont't forget to add @Valid before @RequestBody with corresponding annotations in
     * CreditCardDto to validate credit card fields with javax.validation. (You have to add validation.jar to classpath
     * for it)
     *
     * @param sessionUser
     * @param creditCard

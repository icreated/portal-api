...
/**
 * This class replaces Idempiere Query. 
 * Actually, custom Idempiere ctx not propagated correctly and result methods "first", "firstOnly", "list"
 * loose it, because of calls of Env.getCtx() on low level 
 * The unique solution is extend Query & fix propagation error.
 */
public class PQuery extends Query {
	...
	/**
	 * The buildSQL is private method.
	 * Workaround is calling it by reflection
	 * @param selectClause
	...
	private String buildSqlFix(StringBuilder selectClause, boolean useOrderByClause) {
		try {
			Method buildSql = Query.class.getDeclaredMethod("buildSQL", StringBuilder.class, boolean.class);
	...
	/**
	 * The createResultSet is private method
	 * Workaround is calling it by reflection
	 * @param pstmt
	 * @return
	 */

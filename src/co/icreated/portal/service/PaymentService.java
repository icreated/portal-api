...
public class PaymentService {
    ...
    public void createPayments(SessionUser sessionUser, List<OpenItemDto> openItems, CreditCardDto creditCard,
            String trxName) {

        openItems.stream().forEach(openItem -> createCreditCardPayment(sessionUser, openItem, creditCard, trxName));
    }

    public boolean createCreditCardPayment(SessionUser sessionUser, OpenItemDto openItem, CreditCardDto creditCard,
            String trxName) {
        ...
        MBankAccount ba = new PQuery(ctx, MBankAccount.Table_Name, "AD_Org_ID=? AND C_Currency_ID=?", trxName)
        ...
        MDocType doctype = stream.filter(item -> item.getAD_Org_ID() == Env.getAD_Org_ID(ctx)).findFirst()
                .orElse(doctypes[0]);
    ...
    public MBPBankAccount getBankAccount(SessionUser sessionUser, OpenItemDto openItem, String trxName) {

        MBPartner bp = new MBPartner(ctx, sessionUser.getPartnerId(), trxName);
        return Stream.of(bp.getBankAccounts(false)).filter(ba -> ba.getAD_User_ID() == sessionUser.getUserId())
                .findAny().orElseGet(() -> {
